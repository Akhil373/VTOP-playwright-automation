...
def solve_captcha_with_gemini(image_path: str) -> str:
    print("Asking Gemini to solve the CAPTCHA...")
    
    client: genai.client.Client = genai.Client()
    img: Image.Image = Image.open(image_path)
    prompt = "Read the characters in this CAPTCHA image. Provide only the characters, with no explanation. Hint: The captcha is ALWAYS capital alphabets and numbers."
...
def solve_with_retry(file_path) -> str: 
    ...
    print(f"\nFailed to solve the CAPTCHA with OCR Model. Trying with Gemini...")
    input_str = solve_captcha_with_gemini(file_path)
    return input_str
